...
def checkvirtualenv():
    ...
    checks to make sure that pyglet is installed
    """
    try:
        import virtualenv
    except ImportError:
    ...
    try:
        ...
        log("Warning: pyglet python module not found, trying with pip")
        try:
            import pip
        except:
            log("Warning: pip python module not found, Not sure what to do now.")
            exit(1)
        cmd = "pip install pyglet"
        subprocess.call( cmd.split( " " ) )
...
def install_deps(tmpdir = "", test = False):
    ...
    if sys.platform.startswith('win32'):
        ...
        targetDir = "bin"
        cmd = "%s install coverage nose pyglet" %( os.path.join(tmpdir, targetDir, "pip") )
...
def run_test(test = False, tmpdir = "", dep_return = "", nose = False, nose_coverage_html = "/tmp/pythonUnitCoverage"):
    ...
    if sys.platform.startswith('win32'):
        ...
        if nose:
            ...
            unit_cmd = "%s -v --cover-package=viz3d --cover-tests --with-coverage --cover-erase --with-xunit --exe %s" \
